...
import type { SearchData } from "@/shared/api/backend";
import { KIND_LABEL } from "@/features/wiki/lib/tree";
...
export default function SearchResultsView({ query, data, failed }: {
  query: string; data: SearchData | null; failed: boolean;
}) {
  return (
    <div>
      <Header />
      <main style={{ maxWidth: 800, margin: "0 auto", padding: "2rem 1.5rem" }}>
        ...
        {!query && <p style={{ color: "var(--muted)" }}>검색어를 입력하세요.</p>}
        {failed && <p style={{ color: "var(--muted)" }}>검색 서버에 연결할 수 없습니다. 잠시 후 다시 시도해주세요.</p>}
        {data && (
          <div>
